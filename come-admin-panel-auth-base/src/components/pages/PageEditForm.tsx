...
export default function PageEditForm({ id, initialData }: PageEditFormProps) {
  ...
  useEffect(() => {
    api.get<LinkableComponent[]>("/components/my").then((res) => setAvailableComponents(res.data));
  }, []);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (selectedComponentIds.length > 0) {
        await api.post(`/pages/${id}/bulk-link`, {
      ...
      const message = axiosErr.response?.data?.message ?? "❌ Failed to update page";
      toast.error(message);
