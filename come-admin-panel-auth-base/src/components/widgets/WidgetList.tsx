...
import { duplicateWidget } from "@/lib/api";
...
function formatValue(key: string, value: unknown): string {
  ...
  if (key === "pages") return `${(value as unknown[]).length} page(s)`;
...
export default function WidgetList({ widgets, onDelete }: WidgetListProps) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const message = axiosErr.response?.data?.message ?? "❌ Failed to delete widget";
      toast.error(message);
    } finally {
      setDeletingId(null);
    }
  ...
  const handleDuplicate = async (id: number) => {
    try {
      const duplicated = await duplicateWidget(id);
  ...
  if (widgets.length === 0) {
    return (
      <div className="text-center text-[#7d6f61] mt-8">
        ...
        <Link href="/widgets/create" className="mt-4 inline-block text-[#a67857] hover:underline">
