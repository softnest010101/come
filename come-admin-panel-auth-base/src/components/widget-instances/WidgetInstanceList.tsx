...
import { duplicateWidgetInstance } from "@/lib/api";
import UniversalLinkModal from "@/shared/UniversalLinkModal";

type WidgetInstanceItem = Record<string, unknown>;

type WidgetInstanceListProps = {
  widgetInstances: WidgetInstanceItem[];
...
export default function WidgetInstanceList({
  widgetInstances,
  onDelete,
}: WidgetInstanceListProps) {
  ...
  const allFields = useMemo(() => {
    ...
    widgetInstances.forEach((widget) => {
      Object.keys(widget).forEach((key) => fieldSet.add(key));
    });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await onDelete(id);
      toast.success("✅ WidgetInstance deleted");
    } catch (err: unknown) {
      const axiosErr = err as AxiosError<{ message?: string }>;
      const message = axiosErr.response?.data?.message ?? "❌ Failed to delete";
      toast.error(message);
    } finally {
      setDeletingId(null);
    }
  ...
  const handleDuplicate = async (id: number) => {
    try {
      const duplicated = await duplicateWidgetInstance(id);
  ...
  if (widgetInstances.length === 0) {
    return (
      <div className="text-center text-[#7d6f61] mt-8">
        <p>No widget instances found.</p>
        <Link
          href="/widget-instances/create"
          className="mt-4 inline-block text-[#a67857] hover:underline"
        >
      ...
      <table className="min-w-full border border-[#dfcbb9] rounded-md text-sm">
        ...
        <tbody>
          {widgetInstances.map((widget, index) => (
            <tr
              key={widget.id as number}
              className="hover:bg-[#fbf6ef] text-[#3e3e3e]"
            >
              ...
              <td className="px-2 py-2 border">
                <div className="flex flex-row flex-wrap gap-1 justify-end items-center">
                  <button
                    onClick={() => router.push(`/widget-instances/${widget.id}`)}
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDelete(widget.id as number);
                    }}
                  ...
                  >
                    {deletingId === widget.id ? "Deleting..." : "🗑️ Delete"}
                  </button>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDuplicate(widget.id as number);
                    }}
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setLinkingId(widget.id as number);
                    }}
