...
type ComponentItem = Record<string, unknown>;
...
export default function ComponentList({ components, onDelete }: ComponentListProps) {
  ...
  const allFields = useMemo(() => {
    ...
    components.forEach((component) => {
      Object.keys(component).forEach((key) => fieldSet.add(key));
    });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const message = axiosErr.response?.data?.message ?? "❌ Failed to delete component";
      toast.error(message);
    } finally {
      setDeletingId(null);
    }
  ...
  const handleDuplicate = async (id: number) => {
    try {
      const duplicated = await duplicateComponent(id);
  ...
  if (components.length === 0) {
    return (
      <div className="text-center text-[#7d6f61] mt-8">
        ...
        <Link href="/components/create" className="mt-4 inline-block text-[#a67857] hover:underline">
            ...
            >
              ...
              <td className="px-3 py-2 border text-right space-x-2">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDelete(component.id as number);
                  }}
                ...
                >
                  {deletingId === component.id ? "Deleting..." : "🗑️ Delete"}
                </button>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDuplicate(component.id as number);
                  }}
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setLinkingId(component.id as number);
                  }}
