...
export default function ComponentEditForm({ id, initialData }: ComponentEditFormProps) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (selectedPageIds.length > 0) {
        await api.post(`/components/${id}/bulk-link`, {
          targetModel: "page",
      ...
      const message = axiosErr.response?.data?.message ?? "❌ Failed to update component";
      toast.error(message);
