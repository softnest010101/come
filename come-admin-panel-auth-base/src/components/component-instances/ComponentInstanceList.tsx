...
export default function ComponentInstanceList({
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await onDelete(id);
      toast.success("✅ Component Instance deleted");
    } catch (err: unknown) {
  ...
  const handleDuplicate = async (id: number) => {
    try {
      const duplicated = await duplicateComponentInstance(id);
  ...
  if (instances.length === 0) {
    return (
      <div className="text-center text-[#7d6f61] mt-8">
        ...
        >
          ➕ Create your first component instance
        </Link>
      </div>
    );
  }
    ...
    <div className="overflow-x-auto">
        ...
        <tbody>
          {instances.map((item, index) => (
            <tr
              ...
              onClick={() => router.push(`/component-instances/${item.id}`)}
            >
              ...
              <td className="px-3 py-2 border text-right space-x-2">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    router.push(`/component-instances/edit/${item.id}`);
                  }}
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDuplicate(item.id as number);
                  }}
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setLinkingId(item.id as number);
                  }}
      ...
      {linkingId !== null && (
        <UniversalLinkModal
          isOpen={true}
          sourceId={linkingId}
          sourceModel="component-instance"
