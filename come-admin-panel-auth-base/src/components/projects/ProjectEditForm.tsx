...
export default function ProjectEditForm({ id, initialData }: ProjectEditFormProps) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (selectedPageIds.length > 0) {
        await api.post(`/projects/${id}/bulk-link`, {
          targetModel: "page",
      ...
      const message = axiosErr.response?.data?.message ?? "❌ Failed to update project";
      toast.error(message);
