// 📁 my-platform/come-admin-panel-auth-base/src/app/(protected)/widgets/page.tsx
...
export default function WidgetsPage() {
  ...
  const fetchWidgets = async () => {
    try {
      const res = await api.get<Widget[]>("/widgets/my");
      setWidgets(res.data);
    } catch (err) {
  ...
  useEffect(() => {
    let result = [...widgets];
    if (visibleOnly) result = result.filter((w) => w.visible !== false);
    if (search.trim()) {
  ...
  }, [widgets, search, sortAsc, visibleOnly]);
  ...
  return (
    <div className="min-h-screen bg-[#fdfaf4] px-6 py-8 font-sans text-[#3e3e3e]">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-serif font-bold text-[#5b4636]">🧩 Widgets</h1>
      </div>
      ...
      <div className="bg-white border border-[#dfcbb9] rounded-md overflow-x-auto shadow-md">
        {loading ? (
          <p className="p-4 text-center italic text-[#8b7b6b]">Loading widgets...</p>
        ) : (
          <WidgetList widgets={paginatedWidgets} onDelete={handleDelete} />
        )}
      </div>
